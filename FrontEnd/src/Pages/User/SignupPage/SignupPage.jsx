...
function SignupPage() {
  ...
  const signupHandle = async (data) => {
    ...
    if (
      ...
      !handleFileValidation(data.profilepicture[0])
    ) {
      return;
    }
    ...
    try {
      ...
      const response =await publicaxiosconfig.post("/userSignup", formData, {
        headers: {
      ...
      navigate("/confirmOtp",{state:{'email':response.data.email}});
    } catch (error) {
      console.log(error);
          ...
          <form onSubmit={handleSubmit(signupHandle)}>
            ...
            <div className="mb-3">
              ...
              {validationErrors.profilepicture && (
                <p className="text-danger">
                  {validationErrors.profilepicture.message}
                </p>
              )}
            ...
            <button
              type="submit"
              className="rounded bg-green-900 w-full text-white"
            >
              Sign Up
            </button>
          </form>
