  ...
  const loginSubmitHandler = async (data) => {
    try {
      ...
      if (response.data.user.is_verified) {
        ...
        navigate(response.data.user.is_admin ? "/salesReport" : "/HomePage");
      } else {
        toast.info("Verifiy your Email first ", {
          position: "bottom-center",
        });
        try {
          await publicaxiosconfig.post("/forgetPassword", { email: userEmail });
          navigate("/confirmOtp", { state: { email: userEmail } });
        } catch (error) {
      ...
      <div className="container-fluid d-flex justify-content-center align-items-center bg-[#FCF4D2] h-[70vh]">
        ...
        >
          ...
          <form onSubmit={handleSubmit(loginSubmitHandler)}>
            ...
            <button
              type="submit"
              className="rounded bg-[#467927] text-white w-100"
            >
              Sign in
            </button>
          </form>
          ...
          <div className="text-center mt-3">
            <Link to="/ForgetPassword" className="text-[#BF923F] fw-bold">
              Forgot Password?
            </Link>
          </div>
